...
const SeriesCalculator: React.FC = () => {
  ...
  return (
    <Paper elevation={2} sx={{ p: 2, maxWidth: 600, mx: 'auto' }}>
      <Typography variant="h6" gutterBottom>Series Expansion Calculator</Typography>
      <Grid container spacing={2}>
        ...
        <Grid item xs={4}>
          <TextField fullWidth type="number" label="Terms" value={terms} onChange={e => setTerms(parseInt(e.target.value))} />
        </Grid>
        <Grid item xs={4}>
          <TextField fullWidth type="number" label="Point" value={point} onChange={e => setPoint(parseFloat(e.target.value))} />
        </Grid>
        <Grid item xs={12} sm={6}>
          <FormControl fullWidth>
            <InputLabel>Series Type</InputLabel>
            <Select value={type} label="Series Type" onChange={e => setType(e.target.value as SeriesType)}>
        ...
        {type === 'fourier' && (
          <Grid item xs={12} sm={6}>
            <TextField fullWidth type="number" label="Period" value={period} onChange={e => setPeriod(parseFloat(e.target.value))} />
          </Grid>
        )}
        {type === 'laurent' && (
          <>
            <Grid item xs={6}>
              <TextField fullWidth type="number" label="Inner Radius" value={inner} onChange={e => setInner(parseFloat(e.target.value))} />
            </Grid>
            <Grid item xs={6}>
              <TextField fullWidth type="number" label="Outer Radius" value={outer} onChange={e => setOuter(parseFloat(e.target.value))} />
            </Grid>
          </>
        )}
        <Grid item xs={12}>
      ...
      {result.length > 0 && (
        <Box sx={{ mt: 2 }}>
          ...
          <Typography variant="body2" sx={{ whiteSpace: 'pre-line' }}>{result.join(' + \n')}</Typography>
        </Box>
      )}
    </Paper>
  );
